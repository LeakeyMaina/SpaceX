...
describe("[Query.launches]", () => {
  const mockContext = {
    dataSources: {
      LaunchDataSource: { getAllLaunches: jest.fn() }
    }
  };
  // just for easy access
  const { getAllLaunches } = mockContext.dataSources.LaunchDataSource;
...
describe("[Query.launch]", () => {
  const mockContext = {
    dataSources: {
      LaunchDataSource: { getLaunchById: jest.fn() }
    }
  };

  it("calls lookup from launch api", async () => {
    const getLaunchById = mockContext.dataSources.LaunchDataSource.getLaunchById;
...
describe("[Query.me]", () => {
  const mockContext = {
    dataSources: {
      UserDataSource: { findOrCreateUser: jest.fn() }
    },
    user: {}
  };
  ...
  it("returns user from UserDataSource", async () => {
    mockContext.user.email = "a@a.a";
    const findOrCreateUser =
      mockContext.dataSources.UserDataSource.findOrCreateUser;
